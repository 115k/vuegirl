...
export default class CanvasNest {
  ...
  bindEvent() {
    ...
    window.ontouchmove = throttle(e => {
      ...
      this.current.x = target.clientX - this.el.offsetLeft + document.scrollingElement.scrollLeft; // 当存在横向滚动条时，x坐标再往右移动滚动条拉动的距离
      this.current.y = target.clientY - this.el.offsetTop + document.scrollingElement.scrollTop; // 当存在纵向滚动条时，y坐标再往下移动滚动条拉动的距离
      this.ontouchmove && this.ontouchmove(e);
    }, 10);

    this.ontouchend = window.ontouchend;
    window.ontouchend = () => {
      this.current.x = null;
      this.current.y = null;
  ...
  destroy() {
    ...
    // mouse 事件清除
    window.ontouchmove = this.ontouchmove; // 回滚方法
